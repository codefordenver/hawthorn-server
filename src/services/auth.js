...
class AuthClient {
  constructor(fusionAuthConfig) {
    ...
    this.client = new FusionAuthClient(fusionAuthConfig.apiKey, fusionAuthConfig.endpoint);
  }
  ...
  getUser(id) {
    return this.client
      .retrieveUser(id)
      .then(clientResponse => clientResponse.successResponse.user)
      .catch(error => {
  ...
  async register(email, password, username) {
    const randomImageUrl = `https://api.adorable.io/avatars/50/${Math.round(
    ...
    if (body.fieldErrors != null) {
      for (const errorKey in body.fieldErrors) {
